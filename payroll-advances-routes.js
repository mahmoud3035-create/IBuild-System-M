...
function registerPayrollAdvanceRoutes(app) {
  // IMPORTANT: these routes are registered by payroll-advances-bootstrap.js
  // before server.js installs its global body parsers. Parse request bodies here.
  app.use('/api/payroll', express.json({ limit: '10mb' }));
  app.use('/api/payroll', express.urlencoded({ extended: true }));
    ...
    try {
      ...
      const [rows] = await db.query(`
        SELECT
          ...
          GREATEST(
            0,
            COALESCE(a.total_advance, 0) - COALESCE(p.total_repaid, 0)
          ) AS balance,
      ...
      const summary = rows.map(r => ({
        ...
        balance: Number(Number(r.balance || 0).toFixed(2)),
  ...
  app.post('/api/payroll/advances', async (req, res) => {
    try {
      ...
      const employeeId = numberValue(
        body.employee_id ?? body.employeeId ?? body.employee ?? body.staff_id
      );
      const amount = numberValue(
        body.amount ?? body.advance_amount ?? body.advanceAmount ?? body.value
      );
      const reason = String(
        body.reason ?? body.advance_reason ?? body.advanceReason ?? ''
      ).trim();
      const advanceDate = String(
        body.advance_date ?? body.advanceDate ?? new Date().toISOString().slice(0, 10)
      ).slice(0, 10);

      if (!employeeId || amount <= 0 || !reason) {
        return res.status(400).json({
          success: false,
          message: 'الموظف وقيمة السلفة وسبب السلفة مطلوبة'
        });
      }
      ...
      const [result] = await db.query(
        `INSERT INTO payroll_advances
         (employee_id, advance_date, amount, reason)
         VALUES (?, ?, ?, ?)`,
  ...
  app.post('/api/payroll/advance-deduction', async (req, res) => {
    try {
      ...
      const body = req.body || {};
      const employeeId = numberValue(body.employee_id ?? body.employeeId);
      const amount = numberValue(body.amount ?? body.deduction_amount);
      const payrollMonth = monthValue(body.payroll_month ?? body.payrollMonth ?? body.month);
      const reason = String(body.reason ?? body.deduction_reason ?? '').trim();
      ...
      const [advanceRows] = await db.query(
        'SELECT COALESCE(SUM(amount),0) AS total FROM payroll_advances WHERE employee_id = ?',
        [employeeId]
      );
      ...
      const balance = Math.max(
        0,
        numberValue(advanceRows[0]?.total) - numberValue(paidRows[0]?.total)
      );

      if (amount > balance + 0.005) {
        return res.status(400).json({
          success: false,
          message: `قيمة الخصم أكبر من رصيد السلف المتبقي (${balance.toFixed(2)} AED)`
        });
      }

      await db.query(
        `INSERT INTO payroll_advance_deductions
         (employee_id, payroll_month, amount, reason)
         VALUES (?, ?, ?, ?)
      ...
      if (payrollRows.length) {
        ...
        await db.query(
          'UPDATE payroll_records SET net_salary = ? WHERE id = ?',
          [net.toFixed(2), row.id]
        );
      }

      return res.json({
        success: true,
        message: 'تم حفظ خصم السلفة لهذا الشهر',
  ...
  app.delete('/api/payroll/advances/:id', async (req, res) => {
    try {
      ...
      const [result] = await db.query(
        'DELETE FROM payroll_advances WHERE id = ?',
        [id]
      );
